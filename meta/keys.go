...
func HostCounterKey() string {
	return filepath.Join(config.Conf.EtcdPrefix, fmt.Sprintf("%s:counter", hostPrefix))
}
...
func HostKey(name string) string {
	return filepath.Join(config.Conf.EtcdPrefix, hostPrefix, name)
}
...
func GuestsPrefix() string {
	return fmt.Sprintf("%s/", filepath.Join(config.Conf.EtcdPrefix, guestPrefix))
}

// VolumeKey /<prefix>/vols/<id>
func VolumeKey(id string) string {
	return filepath.Join(config.Conf.EtcdPrefix, volPrefix, id)
}

func SnapshotKey(id string) string {
	return filepath.Join(config.Conf.EtcdPrefix, snapshotPrefix, id)
}
...
func UserImagePrefix(user string) string {
	return filepath.Join(config.Conf.EtcdPrefix, uimgPrefix, user)
}
...
func SysImagePrefix() string {
	return filepath.Join(config.Conf.EtcdPrefix, imgPrefix)
}
